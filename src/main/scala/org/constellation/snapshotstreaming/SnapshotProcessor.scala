...
import io.constellationnetwork.schema.{CurrencyStateProofSelector, GlobalIncrementalSnapshot, GlobalSnapshot, GlobalSnapshotInfo, GlobalSnapshotInfoV1, GlobalStateProofSelector, SnapshotOrdinal, StateProofSelector}
...
object SnapshotProcessor {
  ...
  def make[F[_]: Async: Parallel: HasherSelector](
    ...
    private def process(globalSnapshotWithState: GlobalSnapshotWithState, hasher: Hasher[F], mptStore: MptStore[F, GlobalStateKey])(
      implicit stateProofSelector: StateProofSelector
    ): F[Unit] = {
      val GlobalSnapshotWithState(snapshot, _, snapshotInfo, _, _) = globalSnapshotWithState
      JsonSerializer.forAsync.map { implicit jsonSerializer =>
        HasherSelector[F]
        ...
        consumer = Stream
          ...
          .evalMap { snapshot =>
            ...
            logger.info(s"Consumer: Processing snapshot ${getSnapshotReference(snapshot.snapshot)}") >>
              retryF(
                process(snapshot, hasher, mptStore).timedLog(s"Consumer: processed snapshot ${snapshot.snapshot.ordinal.value}")
              ).handleErrorWith { e =>
                logger.error(e)(
