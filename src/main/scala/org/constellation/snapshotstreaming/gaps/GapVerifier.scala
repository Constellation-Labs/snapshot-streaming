...
class GapVerifier {
  def hasNonDummyTransactions(snapshotInfoBeforeGap: SnapshotInfo,
                              snapshotInfoAfterGap: SnapshotInfo): Boolean = {
    ...
    val difference
      : Set[(String, Long)] = nonRewardBalancesBeforeGap.toSet diff nonRewardBalancesAfterGap.toSet
    difference.nonEmpty
  }
}
