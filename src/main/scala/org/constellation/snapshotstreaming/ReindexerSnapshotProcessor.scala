...
import org.tessellation.schema.SnapshotReference.fromHashedSnapshot
import org.tessellation.schema.address.Address
import org.tessellation.schema.{GlobalIncrementalSnapshot, GlobalSnapshot, GlobalSnapshotInfo, GlobalSnapshotInfoV2, SnapshotOrdinal}
...
trait ReindexerSnapshotProcessor[F[_]] {
  val runtime: Stream[F, Unit]
}

object ReindexerSnapshotProcessor {
  ...
  def make[F[
    ...
    txHasher: Hasher[F]
  ): Resource[F, ReindexerSnapshotProcessor[F]] =
    for {
  ...
  def make[F[_]: Async: Parallel: HasherSelector](
  ...
  ): ReindexerSnapshotProcessor[F] = new ReindexerSnapshotProcessor[F] {
    ...
    private def splitData(globalSnapshotWithState: GlobalSnapshotWithState) = {
      implicit val hasher = HasherSelector[F].getForOrdinal(globalSnapshotWithState.snapshot.ordinal)
      (
    ...
    private def store(globalSnapshotWithStates: Seq[GlobalSnapshotWithState]): F[Unit] =
      globalSnapshotWithStates
        .traverse(splitData)
        .flatMap { seqs =>
          val (globalData, metagraphData) = seqs.unzip
    ...
    def getGlobalSnapshotByOrdinal(
      ordinal: SnapshotOrdinal
    )(implicit hs: HasherSelector[F]): F[Option[Hashed[GlobalIncrementalSnapshot]]] = {
      ...
      opensearchDAO
        .singleQuery(q, hitMapper)
        .flatMap(_.traverse { case (hash, _) =>
              ...
              logger.info(s"Downloading hash ${h} from S3") >>
                s3DAO
                  ...
                  .flatMap { snapshot =>
                    ...
                    logger.info(s" ordinal ${snapshot.ordinal} for hash ${h} timestamped at $ts") >>
                    snapshot.toHashed[F]
                  }
                  .map((h, _, ts))
            }
