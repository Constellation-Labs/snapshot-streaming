...
object FileBasedLastIncrementalGlobalSnapshotStorageSuite extends MutableIOSuite {
  ...
  private def mkInitialSnapshot()(implicit ks: KryoSerializer[IO]) =
    incrementalGlobalSnapshot(100L, 10L, 20L, Hash("abc"), Hash("def"), snapshotInfo)

  test("get should return None before initial snapshot is set") { implicit ks =>
  ...
  test("getCombined should return None before initial snapshot is set") { implicit ks =>
    fileBasedStorage.use { storage =>
      storage.get
        .map(expect.same(None, _))
    }
  ...
  test("get should return last snapshot after it's set") { implicit ks =>
    fileBasedStorage.use { storage =>
  ...
  test("getCombined should return last snapshot after it's set") { implicit ks =>
    fileBasedStorage.use { storage =>
      mkInitialSnapshot().flatMap { initial =>
        storage.setInitial(initial, snapshotInfo) >>
          storage.getCombined
  ...
  test("getOrdinal should return None before initial snapshot is set") { implicit ks =>
    fileBasedStorage.use { storage =>
      storage.getOrdinal
        .map(expect.same(None, _))
    }
  ...
  test("getOrdinal should return last snapshot's ordinal after it's set") { implicit ks =>
    fileBasedStorage.use { storage =>
      mkInitialSnapshot().flatMap { initial =>
        storage.setInitial(initial, snapshotInfo) >>
          storage.getOrdinal
  ...
  test("getHeight should return None before initial snapshot is set") { implicit ks =>
    fileBasedStorage.use { storage =>
      storage.getHeight
        .map(expect.same(None, _))
    }
  ...
  test("getHeight should return last snapshot's height after it's set") { implicit ks =>
    fileBasedStorage.use { storage =>
      mkInitialSnapshot.flatMap { initial =>
        storage.setInitial(initial, snapshotInfo) >>
          storage.getHeight
  ...
  test("set should fail when we try to set snapshot before the initial snapshot is set") { implicit ks =>
    fileBasedStorage.use { storage =>
      mkInitialSnapshot.flatMap { initial =>
        storage
  ...
  test("set should fail when we try to set a snapshot that's not the next one") { implicit ks =>
    fileBasedStorage.use { storage =>
      mkInitialSnapshot.flatMap { initial =>
  ...
  test("set should fail when we try to set a snapshot with not matching state") { implicit ks =>
    fileBasedStorage.use { storage =>
      mkInitialSnapshot.flatMap { initial =>
  ...
  test("set should successfully set a snapshot if it is the next one") { implicit ks =>
    fileBasedStorage.use { storage =>
      mkInitialSnapshot.flatMap { initial =>
        incrementalGlobalSnapshot(101L, 10L, 21L, Hash("def"), Hash("ghi"), snapshotInfo).flatMap { nextCorrect =>
          storage.setInitial(initial, snapshotInfo) >>
  ...
  test("setInitial should fail if the initial snapshot already exists") { implicit ks =>
    fileBasedStorage.use { storage =>
      mkInitialSnapshot.flatMap { initial =>
        storage.setInitial(initial, snapshotInfo) >>
          storage
  ...
  test("setInitial should fail when we try to set initial snapshot with not matching state") { implicit ks =>
    fileBasedStorage.use { storage =>
      mkInitialSnapshot.flatMap { initial =>
        storage
  ...
  test("setInitial should successfully set initial snapshot if it not yet exists") { implicit ks =>
    fileBasedStorage.use { storage =>
      mkInitialSnapshot.flatMap { initial =>
        storage
          .setInitial(initial, snapshotInfo)
