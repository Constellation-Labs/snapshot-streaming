...
object CurrencySnapshotMapperSuite extends MutableIOSuite {
  ...
  test("set balance to 0 for source when not in info") { res =>
    ...
    for {
      ...
      feeTxn1 <- createFeeTxn(address3, key3, address4)
      feeTxn2 <- createFeeTxn(address4, key4, address3)
      feeTxn3 <- createFeeTxn(address4, key4, address7)
      ...
      feeTransactions = SortedSet(feeTxn1, feeTxn2, feeTxn3).some
  ...
  test("leave balances for addresses from transactions and fee transactions") { res =>
    implicit val (h, ks, sp, key1, key2, key3, key4) = res
    val address1 = key1.getPublic.toAddress
    val address2 = key2.getPublic.toAddress
    val address3 = key3.getPublic.toAddress
    val address4 = key4.getPublic.toAddress
    val balances = createBalances(address1, address2, address3, address4, address5, address6, address7)
    ...
    for {
      ...
      result = CurrencyIncrementalSnapshotMapper.make().snapshotReferredBalancesInfo(snapshot, totalInfo)
      expectedBalances = createBalances(address1, address2, address3, address4, address5, address7)
    } yield expect.same(result, expectedBalances)
  }

  test(
    "leave balances for addresses from transactions and fee transactions, but not set zero for destinations not in balances "
  ) { res =>
    implicit val (h, ks, sp, key1, key2, key3, key4) = res
    val address1 = key1.getPublic.toAddress
    val address2 = key2.getPublic.toAddress
    ...
    for {
      ...
      snapshot <- incrementalCurrencySnapshot[IO](100L, 10L, 20L, Hash("abc"), Hash("def"), totalInfo, blocks, feeTransactions=feeTransactions)

      result = CurrencyIncrementalSnapshotMapper.make().snapshotReferredBalancesInfo(snapshot, totalInfo)
      expectedBalances = SortedMap(address1 -> Balance(1000L), address2 -> Balance(0L), address3 -> Balance(1000L), address4 -> Balance(0L))
    } yield expect.same(result, expectedBalances)
  }
